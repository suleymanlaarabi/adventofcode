...
fn apply_rules(mut vec: Vec<i32>, rules: &Vec<Rule>) -> Vec<i32> {
    for rule in rules {
        if let Some(before_index) = vec.iter().position(|&x| x == rule.before) {
            if let Some(after_index) = vec.iter().position(|&x| x == rule.after) {
                if before_index > after_index {
                    vec.swap(before_index, after_index);
                    vec = apply_rules(vec, rules);
                }
            }
        }
    }
...
fn main() -> io::Result<()> {
    ...
    for page in pages {
        if !valide_page(&page, &rules) {
            let sorted = apply_rules(page, &rules);
            result += get_middle(&sorted);
        }
    }
    println!("{}", result);
    Ok(())
